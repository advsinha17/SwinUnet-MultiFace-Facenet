...
class WindowAttention(tf.keras.layers.Layer):
    ...
    def call(self, input, mask = None):
        ...
        qkv = self.qkv(input)
        qkv = tf.reshape(qkv, [3, num_batch_windows, self.num_heads, num_patches, C // self.num_heads])
        ...
        if mask:
            ...
            attn = tf.reshape(attn, [num_batch_windows // num_windows, num_windows, self.num_heads, num_patches, num_patches]) + tf.expand_dims(tf.expand_dims(mask, 1), 0)
...
class TransformerBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, 
        ...
        if self.window_shift > 0:
            ...
            mask = tf.zeros((1, height, width, 1))
            h_slices = (
