...
class ConvNeXTUNet(tf.keras.Model):
    ...
    def __init__(self,  input_dim, embed_dim, n_classes = 3, depths = [2, 2, 6, 2]):
    ...
    def call(self, x, training = False):
        ...
        for layer in self.stage3:
            x = layer(x)
        down2 = tf.reshape(down2, [-1, self.input_dim // (2 * 4), self.input_dim // (2 * 4), 2 * self.embed_dim])
        for layer in self.downsampling_block3:
            x = layer(x, training = training)
        down3 = x
        for layer in self.stage4:
            x = layer(x)
        down3 = tf.reshape(down3, [-1, self.input_dim // (4 * 4), self.input_dim // (4 * 4), 4 * self.embed_dim])
        ...
        for layer in self.downsampling_block5:
            x = layer(x, training = training)
        down5 = x
        down4 = tf.reshape(down4, [-1, self.input_dim // (8 * 4), self.input_dim // (8 * 4), 8 * self.embed_dim])
        down5 = tf.reshape(down5, [-1, self.input_dim // (8 * 4), self.input_dim // (8 * 4), 8 * self.embed_dim])
